  ...
  _createClass(Clamp, [{
    ...
    value: function clamp() {
      var _props = this.props,
          ...
          splitByWords = _props.splitByWords,
      ...
      if (currentHeight > maxHeight) {
        ...
        var trunk = splitByWords ? text.match(/\w+|\W+?/g) : text;
    ...
    key: 'trunkSlice',
    value: function trunkSlice(trunk, maxHeight, increase, len, isDecrease) {
      var _props2 = this.props,
          reverse = _props2.reverse,
          splitByWords = _props2.splitByWords;

      var slicedTrunk = reverse ? trunk.slice(trunk.length - len) : trunk.slice(0, len);

      (0, _utils.setText)(this.content, splitByWords ? slicedTrunk.join('') : slicedTrunk);
      ...
      if ((0, _utils.getHeight)(this.wrapper) > maxHeight) {
        ...
        if (increase === 1 && isDecrease) {
          if (splitByWords && /\s/.test(slicedTrunk[reverse ? 0 : slicedTrunk.length - 1])) {
            (0, _utils.setText)(this.content, (reverse ? slicedTrunk.slice(1) : slicedTrunk.slice(0, slicedTrunk.length - 1)).join(''));
          }
          return;
        }
